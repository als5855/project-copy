...
public class RecommendationController {
    ...
    @PostMapping
        public ResponseEntity<ResponseDto<ResponseRecommendationDto>> createRecommendation(
                @AuthenticationPrincipal @RequestBody RequestRecommendationDto dto) {

        ResponseDto<ResponseRecommendationDto> response = recommendationService.createRecommendation(dto);
    ...
    @GetMapping
    public ResponseEntity<ResponseDto<List<GetResponseRecommendationDto>>> getRecommendation(RequestRecommendationDto dto) {
        ResponseDto<List<GetResponseRecommendationDto>> response = recommendationService.getRecommendation(dto);
        HttpStatus status = response.isResult() ? HttpStatus.OK:HttpStatus.BAD_REQUEST;
        return ResponseEntity.status(status).body(response);
    }
}
